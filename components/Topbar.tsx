...
export default function Topbar({
  ...
  const profileSeed = useMemo(
    ...
    [profileAvatarUrl, profileName, profilePicId, profileUsername, userId],
  );
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      ...
      try {
        ...
        if (mounted) setAccountRecord(status?.profile || null);
      } catch {
        if (mounted) setAccountRecord(null);
      }
    };
    void loadProfile();
    return () => {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        const result = await AppwriteService.searchGlobalProfiles(text, 5);
        if (!active) return;
        const rows = Array.isArray(result)
          ? result
              .map((candidate: any) => ({
                id: candidate.$id || candidate.userId,
                userId: candidate.userId || candidate.$id || null,
                username: candidate.username || null,
                displayName: candidate.displayName || candidate.name || candidate.username || candidate.email || 'Person',
                avatar: candidate.avatar || candidate.avatarUrl || candidate.avatarFileId || null,
                email: candidate.email || null,
              }))
              .filter((candidate: any) => candidate.userId !== userId)
          : [];
        ...
        if (active) {
          console.warn('[Topbar] People search failed:', error);
          setPeopleResults([]);
        }
      } finally {
  ...
  }, [previewManager, searchOpen, searchQuery, userId]);
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
            <Box sx={{ width: 38, height: 38, borderRadius: '14px', display: 'grid', placeItems: 'center', bgcolor: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)', flexShrink: 0 }}>
              <Logo app="accounts" size={18} variant="icon" />
            </Box>
            <TextField
              ...
              <Box sx={{ display: 'grid', gap: 1.25, maxHeight: '58vh', overflowY: 'auto', pr: 0.5, pb: 0.5 }}>
                ...
                <Box sx={{ borderRadius: '22px', border: '1px solid rgba(255,255,255,0.05)', bgcolor: 'rgba(255,255,255,0.02)', p: 1.5 }}>
                  ...
                  <Typography sx={{ color: 'white', fontSize: '0.88rem', lineHeight: 1.55, wordBreak: 'break-word' }}>
                    {profileUsername ? `@${String(profileUsername).replace(/^@+/, '')}` : (userEmail || 'No username set.')}
                  </Typography>
                </Box>
        ...
        >
          ...
          {activePanel === 'search' && (
            <Stack spacing={1.25}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
                <Box sx={{ width: 38, height: 38, borderRadius: '14px', display: 'grid', placeItems: 'center', bgcolor: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)', flexShrink: 0 }}>
                  <Logo app={logoApp as any} size={18} variant="icon" />
                </Box>
                <TextField
                  inputRef={searchInputRef}
                  value={searchQuery}
        ...
        <Box sx={{ maxWidth: 1440, mx: 'auto', px: { xs: 2, md: 4 }, width: '100%' }}>
          ...
          >
            ...
            <Stack direction="row" alignItems="center" spacing={1.25} sx={{ flexShrink: 0 }}>
              {!userId && (
                <Tooltip title={authLoading ? 'Checking account...' : 'Connect'}>
                  <Button
                    onClick={() => {
                  ...
                  <Tooltip title="Wallet">
                    <IconButton
                      ...
                      sx={{
                        ...
                        border: '1px solid',
                        borderColor: alpha(getAppColor('accounts'), 0.1),
                        borderRadius: '12px',
                        width: 42,
                        height: 42,
                        '&:hover': { bgcolor: alpha(getAppColor('accounts'), 0.08) },
                      }}
                    >
                  ...
                  >
                    <Avatar
                      ...
                      sx={{
                        width: 38,
                        height: 38,
                        bgcolor: profileAvatarUrl ? 'rgba(255,255,255,0.04)' : getAppTone('accounts').secondary,
                        color: '#fff',
                        fontWeight: 900,
                        borderRadius: '12px',
                      }}
